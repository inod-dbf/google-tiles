...
class OGC3DTile extends THREE.Object3D {
    ...
    constructor(properties) {
        ...
        if (this.occlusionCullingService) {
            ...
            this.colorID =
                (clamp(self.color.r * 255, 0, 255) << 16) ^
                (clamp(self.color.g * 255, 0, 255) << 8) ^
                (clamp(self.color.b * 255, 0, 255) << 0);
        }
        if (this.static) {
                ...
                .then((result) => {
                    ...
                    result.json().then((json) => {
                        ...
                        if (properties.onLoadCallback)
                            properties.onLoadCallback(self);
                        if (!!self.centerModel) {
                            ...
                            if (self.boundingVolume instanceof OBB) {
                                ...
                                self.boundingVolume instanceof THREE.Sphere
                            ) {
                                //sphere
                            ...
                            self.translateX(
                                -tempSphere.center.x * self.scale.x
                            );
                            self.translateY(
                                -tempSphere.center.y * self.scale.y
                            );
                            self.translateZ(
                                -tempSphere.center.z * self.scale.z
                            );
                        }
                    });
                })
    ...
    setup(properties) {
        if (!!properties.json.root) {
            ...
            if (!this.json.refinement)
                this.json.refinement = properties.json.refinement;
            if (!this.json.geometricError)
                this.json.geometricError = properties.json.geometricError;
            if (!this.json.transform)
                this.json.transform = properties.json.transform;
            if (!this.json.boundingVolume)
                this.json.boundingVolume = properties.json.boundingVolume;
        } else {
            this.json = properties.json;
        }
        this.rootPath = !!properties.json.rootPath
        ...
        if (!!this.json.boundingVolume) {
            if (!!this.json.boundingVolume.box) {
                ...
                this.boundingVolume = new THREE.Sphere(
        ...
        if (!!this.json.content) {
            ...
            if (
                !!this.json.content.uri &&
                this.json.content.uri.includes("json")
            ) {
                this.hasUnloadedJSONContent = true;
            } else if (
                !!this.json.content.url &&
                this.json.content.url.includes("json")
            ) {
                this.hasUnloadedJSONContent = true;
            } else {
                    ...
                    try {
                        self.tileLoader.get(
                            ...
                            !self.cameraOnLoad
                                ? () => 0
                                : () => {
                                      return self.calculateDistanceToCamera(
                                          self.cameraOnLoad
                                      );
                                  },
                            () => self.getSiblings(),
    ...
    changeContentVisibility(visibility) {
        ...
        if (!!self.meshContent.traverse) {
            ...
            self.meshContent.scenes.forEach((scene) =>
                scene.traverse(function (element) {
                    if (element.material)
                        setMeshVisibility(element, visibility);
                })
            );
        }
    ...
    calculateUpdateMetric(camera, frustum) {
        ...
        let lambda =
            2.0 *
            Math.tan(0.5 * fov * 0.01745329251994329576923690768489) *
            distance;
    ...
    calculateDistanceToCamera(camera) {
        ...
        // console.log('SPHERE RADIUS::::', tempSphere.radius);
        return Math.max(
            0,
            camera.position.distanceTo(tempSphere.center) - tempSphere.radius
        );
